# Vendored from https://github.com/tadfisher/flake (nixos/modules/pia-vpn.nix)
# Original author: Tad Fisher — MIT licence
# Changes made for vex-vpn:
#   - Added dnsServers option defaulting to PIA's own DNS (10.0.0.241, 10.0.0.242)
#     instead of hardcoded Google DNS (8.8.8.8 / 8.8.4.4)
#   - iproute → iproute2 rename already present in upstream; kept as-is
{ config, lib, pkgs, ... }:
let
  ...
  options.services.pia-vpn = {
    ...
    certificateFile = mkOption {
      ...
      description = ''
        ...
        This is provided as ca.rsa.4096.crt.
      '';
    };
    ...
    maxLatency = mkOption {
      ...
      default = 0.1;
      description = ''
        Maximum latency to allow for auto-selection of VPN server, in seconds.
      '';
    };

    # Fix: default to PIA's own DNS instead of hardcoded Google DNS.
    dnsServers = mkOption {
      type = types.listOf types.str;
      default = [ "10.0.0.241" "10.0.0.242" ];
      description = ''
        DNS servers to use while the VPN tunnel is active.
        Defaults to PIA's own DNS servers (10.0.0.241, 10.0.0.242).
        Set to [ "8.8.8.8" "8.8.4.4" ] to use Google DNS, etc.
      '';
    };
    ...
    preDown = mkOption {
      type = types.lines;
      default = "";
      description = "Commands called before the interface is taken down.";
    };

    postDown = mkOption {
      type = types.lines;
      default = "";
      description = "Commands called after the interface is taken down.";
    };
    ...
    portForward = {
      ...
      script = mkOption {
        ...
        default = "";
        description = ''
          Script to execute, with $port set to the forwarded port.
        '';
      };
    };
  };
